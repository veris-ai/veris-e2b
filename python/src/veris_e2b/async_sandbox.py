"""The non-blocking Veris Sandbox — the same contract as :mod:`veris_e2b.sandbox`
on e2b's ``AsyncSandbox``, for callers already inside an event loop.

Every decision the two make is shared (``_options``, ``network``, ``trust``);
only the awaiting differs.
"""
...
from .control_plane import AsyncControlPlane, EgressCredential, TwinSandbox
from .errors import (
...
from .receipt import probe_canary_async
from .sandbox import _NO_GATEWAY, CA_INSTALL_TIMEOUT_S
from .trust import CA_CERT_PATH, CA_INSTALL_CMD, CA_TOOLING_PROBE, sanitize_trust_env
from .veris_api import AsyncVerisApi, VerisContext
...
class AsyncSandbox(BaseAsyncSandbox):
    ...
    async def create(  # type: ignore[override]
        ...
        **opts: Any,
        ...
        control_plane = AsyncControlPlane(
        ...
        twin = await cls._obtain_twin(control_plane, v, coords.environment_id, ttl_minutes)
        environment_id = coords.environment_id or twin.environment_id

        async def cleanup() -> None:
            if owns_twin:
                try:
                    await control_plane.delete_twin(environment_id, twin.id)
                except VerisError:
                    pass  # TTL is the backstop; the original failure is the story

        try:
            await control_plane.gateway_health()
            credential = await control_plane.mint_egress_credential(twin.environment_id, twin.id)
            if credential is None:
                raise VerisGatewayNotOfferedError(
                    _NO_GATEWAY, phase="credential-mint", veris_sandbox_id=twin.id
                )
            return await cls._create_gateway(
    ...
    @staticmethod
    async def _obtain_twin(
        control_plane: AsyncControlPlane, v: VerisOpts, environment_id: str | None, ttl_minutes: int
    ) -> TwinSandbox:
        """The attached twin, or a freshly provisioned one at the requested state."""
        if v.attach_sandbox_id:
            existing = await control_plane.get_twin(v.attach_sandbox_id)
            if existing is None:
            ...
            if existing.status == "ready":
                return existing
            return await control_plane.wait_ready(v.attach_sandbox_id)
        assert environment_id is not None  # resolve_coordinates guarantees it
        created = await control_plane.create_twin(
            environment_id, ttl_minutes=ttl_minutes, snapshot_id=v.snapshot_id
        )
        try:
            return await control_plane.wait_ready(created.id)
        except BaseException:
            try:
                await control_plane.delete_twin(environment_id, created.id)
            except VerisError:
                pass
            raise
    ...
    async def _create_gateway(
        ...
        *,
        ...
        services = twin.services or await control_plane.services(twin.id)
        ...
        try:
            instance = await super().create(  # type: ignore[misc]
                template,
        ...
        try:
            await _write_ca(instance, credential.ca_pem)
            if opts.install_ca:
                await _install_ca(instance)
            await probe_canary_async(instance, credential.canary_host, twin.id, CA_CERT_PATH)
        except BaseException:
            try:
                await BaseAsyncSandbox.kill(instance)
    ...
    async def reconnect(
        ...
        **opts: Any,
        ...
        """
        instance = await cls.connect(sandbox_id, **opts)  # type: ignore[misc]
        info = await instance.get_info()
        wiring = rehydrate(dict(info.metadata or {}), sandbox_id, api_key, api_base)
        control_plane = AsyncControlPlane(
            api_key=wiring.api_key, api_base=wiring.api_base, sdk_version=SDK_VERSION
        )
        twin = await control_plane.get_twin(wiring.twin_id)
    ...
    async def set_timeout(self, timeout: int, **opts: Any) -> None:  # type: ignore[override]
        """Extend the E2B sandbox and the twin's TTL in lockstep, twin first."""
        try:
            await self._veris_control_plane.extend_ttl(
            ...
            pass
        await super().set_timeout(timeout, **opts)
...
async def _write_ca(sandbox: Any, ca_pem: str) -> None:
    """Drop the CA on disk (so ``curl --cacert`` can use it) without touching the system store."""
    await sandbox.files.write(CA_CERT_PATH, ca_pem, user="root")


async def _install_ca(sandbox: Any) -> None:
    """Trust the Veris CA system-wide: probe tooling, then one root command."""
    probe = await sandbox.commands.run(CA_TOOLING_PROBE, user="root")
    ...
    await sandbox.commands.run(CA_INSTALL_CMD, user="root", timeout=CA_INSTALL_TIMEOUT_S)
